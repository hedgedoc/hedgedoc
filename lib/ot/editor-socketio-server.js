    ...
    socket.on('operation', function (revision, operation, selection) {
        ...
        self.mayWrite(socket, function (mayWrite) {
            ...
            try {
                ...
                logger.error("Operation error from client " + socket.id + ": " + err.message);
                if (!socket._opRetries) socket._opRetries = 0;
                socket._opRetries++;
                setTimeout(function() {
                    var docOut = {
                        str: self.document,
                    ...
                    // Only force-reset after 3 consecutive failures as a last resort.
                    // Without force:true, the client will use its Stale recovery to resync.
                    if (socket._opRetries >= 3) {
                        docOut.force = true;
                        socket._opRetries = 0;
                    }
                    socket.emit('doc', docOut);
