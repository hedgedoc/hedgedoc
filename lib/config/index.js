...
if (!config.get('allowAnonymous') && !config.get('allowAnonymousEdits')) {
  delete permissions.freely
}
if (!(config.get('defaultPermission') in permissions)) {
...
config.load({
  serverURL: (function () {
    const isStandardHTTPsPort = config.get('tls.enable') && config.get('port') === 443
    const isStandardHTTPPort = !config.get('tls.enable') && config.get('port') === 80
    let url = ''
