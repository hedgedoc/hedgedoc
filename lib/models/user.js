    ...
    invalid_password_given: {
      ...
      validate: {
        valid: (invalidityReason) => {
          if (invalidityReason) throw new Error(invalidityReason)
        }
      }
    }
  }, {
      ...
      beforeCreate: async function (instance) {
        ...
        if (email) {
          ...
          if (!instance.get('displayname')) {
            instance.set('displayname', localPart)
          }
          if (!instance.get('username')) {
            let localCandidate = localPart
            let counter = 1
            while (await User.findOne({ where: { username: localCandidate } })) {
              localCandidate = localPart + counter
              counter++
            }
            instance.set('username', localCandidate)
          }
        }
      }
    }
