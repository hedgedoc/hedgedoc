...
// permission types
var userTypes = ['owner', 'signedIn', 'anyone']
    ...
    hooks: {
      beforeCreate: function (note, options) {
        return new Promise(function (resolve, reject) {
          ...
          if (note.ownerId) {
            if (!note.viewableBy) {
              note.viewableBy = config.defaultviewableBy;
            }
            if (!note.editableBy) {
              note.editableBy = config.defaultEditableBy;
            }
          } else {
            note.editableBy = 'anyone';
            note.viewableBy = 'anyone';
          }
          return resolve(note)
        })
      },
