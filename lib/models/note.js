      ...
      beforeCreate: function (note, options) {
        return new Promise(function (resolve, reject) {
          ...
          if (!note.content) {
            ...
            let filePath = null
            if (!note.alias) {
              logger.info(path.join( config.get('defaultNotePath')))
              filePath = config.get('defaultNotePath')
            } else {
              logger.info(path.join(config.get('docsPath'), note.alias + '.md'))
              filePath = path.join(config.get('docsPath'), note.alias + '.md')
            }
            if (Note.checkFileExist(filePath)) {
              ...
              note.content = body
              if (filePath !== config.get('defaultNotePath')) {
          ...
          if (!note.permission) {
            if (note.ownerId) {
              note.permission = config.get('defaultPermission')
            } else {
    ...
    async.series({
      parseNoteIdByAlias: function (_callback) {
        ...
        Note.findOne({
          ...
          if (note) {
            logger.info(path.join(config.get('docsPath'), noteId + '.md'))
            let filePath = path.join(config.get('docsPath'), noteId + '.md')
            if (Note.checkFileExist(filePath)) {
            ...
            var filePath = path.join(config.get('docsPath'), noteId + '.md')
            if (Note.checkFileExist(filePath)) {
              Note.create({
                alias: noteId,
                owner: null,
