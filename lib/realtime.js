...
function finishConnection (socket, noteId, socketId) {
  ...
  if (!user) {
    return failConnection(500, 'user not found', socket)
  }
...
function disconnect (socket) {
    ...
    if (Object.keys(note.users).length <= 0) {
      if (note.server.isDirty) {
        updateNote(note, function (err, _note) {
          ...
          // Check again for online users before clear, because users could reconnect during the async updateNote
          if (Object.keys(note.users).length <= 0) {
  ...
  if (config.debug) {
    logger.debug(util.inspect(notes))
    getStatus(function (data) {
      logger.debug(JSON.stringify(data))
    })
  }
