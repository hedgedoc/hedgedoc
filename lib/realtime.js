...
function checkViewPermission (req, note) {
  if (note.viewableBy === 'owner') return req.user && req.user.logged_in && req.user.id === note.owner
  if (note.viewableBy === 'signedIn') return req.user && req.user.logged_in
  return true
}
...
function connection (socket) {
  ...
  socket.on('viewableBy', function (viewableBy) {
    // need login to do more actions
    if (socket.request.user && socket.request.user.logged_in) {
      var noteId = socket.noteId
      if (!noteId || !notes[noteId]) return
      var note = notes[noteId]
      // Only owner can change viewableBy
      if (note.owner && note.owner === socket.request.user.id) {
        if (viewableBy === 'anyone' && !config.allowAnonymous) return
        note.viewableBy = viewableBy
        models.Note.update({
          ...
          realtime.io.to(note.id).emit('viewableBy', out)
          for (var i = 0, l = note.socks.length; i < l; i++) {
            var sock = note.socks[i]
            if (typeof sock !== 'undefined' && sock) {
              // check view viewableBy
          ...
          return logger.error('update note viewableBy failed: ' + err)
        })
      }
    }
  })
  socket.on('editableBy', function (editableBy) {
    // need login to do more actions
    if (socket.request.user && socket.request.user.logged_in) {
      var noteId = socket.noteId
      if (!noteId || !notes[noteId]) return
      var note = notes[noteId]
      // Only owner can change editableBy
      if (note.owner && note.owner === socket.request.user.id) {
        if (editableBy === 'anyone' && !config.allowAnonymous || !config.allowAnonymousEdits) return
        note.editableBy = editableBy
        models.Note.update({
          ...
          realtime.io.to(note.id).emit('editableBy', out)
        }).catch(function (err) {
          return logger.error('update note editableBy failed: ' + err)
        })
      }
    }
  })
