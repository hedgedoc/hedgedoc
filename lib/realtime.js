...
function secure (socket, next) {
  try {
    ...
    if (handshakeData.headers.cookie) {
      ...
      handshakeData.sessionID = cookieParser.signedCookie(handshakeData.cookie[config.get('sessionName')], config.get('sessionSecret'))
      if (handshakeData.sessionID &&
                handshakeData.cookie[config.get('sessionName')] &&
                handshakeData.cookie[config.get('sessionName')] !== handshakeData.sessionID) {
...
function emitRefresh (socket) {
  ...
  var out = {
    title: note.title,
    docmaxlength: config.get('documentMaxLength'),
    owner: note.owner,
...
function connection (socket) {
  ...
  socket.on('permission', function (permission) {
    ...
    if (socket.request.user && socket.request.user.logged_in) {
      ...
      if (note.owner && note.owner === socket.request.user.id) {
        if (permission === 'freely' && !config.get('allowAnonymous') && !config.get('allowAnonymousEdits')) return
  ...
  socket.on('version', function () {
    socket.emit('version', {
      version: config.get('fullversion'),
      minimumCompatibleVersion: config.get('minimumCompatibleVersion')
    })
  })
