...
CspStrategy.computeDirectives = function () {
  var directives = {}
  mergeDirectives(directives, config.get('csp').directives)
  mergeDirectivesIf(config.get('csp').addDefaults, directives, defaultDirectives)
  mergeDirectivesIf(config.get('useCDN'), directives, cdnDirectives)
  mergeDirectivesIf(config.get('csp').addDisqus, directives, disqusDirectives)
  mergeDirectivesIf(config.get('csp').addGoogleAnalytics, directives, googleAnalyticsDirectives)
...
function addUpgradeUnsafeRequestsOptionTo (directives) {
  if (config.get('csp').upgradeInsecureRequests === 'auto' && config.get('tls.enable')) {
    directives.upgradeInsecureRequests = true
  } else if (config.get('csp').upgradeInsecureRequests === true) {
    directives.upgradeInsecureRequests = true
  }
}

function addReportURI (directives) {
  if (config.get('csp').reportURI) {
    directives.reportUri = config.get('csp').reportURI
  }
}
