...
exports.createPDF = function createPDF (req, res, note) {
  const url = config.get('serverURL') || 'http://' + req.get('host')
  const body = note.content
  const extracted = models.Note.extractMeta(body)
  let content = extracted.markdown
  const title = models.Note.decodeTitle(note.title)

  if (!fs.existsSync(config.get('tmpPath'))) {
    fs.mkdirSync(config.get('tmpPath'))
  }
  const path = config.get('tmpPath') + '/' + Date.now() + '.pdf'
...
exports.createGist = function createGist (req, res, note) {
  const data = {
    ...
    redirect_uri: config.get('serverURL') + '/auth/github/callback/' + models.Note.encodeNoteId(note.id) + '/gist',
    scope: 'gist',
