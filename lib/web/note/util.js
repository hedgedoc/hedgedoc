...
exports.newNote = function (req, res, body) {
  ...
  if (req.isAuthenticated()) {
    owner = req.user.id
  } else if (!config.get('allowAnonymous')) {
    return errors.errorForbidden(res)
  }
  if (config.get('allowFreeURL') && noteId && !config.get('forbiddenNoteIDs').includes(noteId)) {
    req.alias = noteId
  } else if (noteId) {
  ...
  models.Note.create({
    ...
    return res.redirect(config.get('serverURL') + '/' + (note.alias ? note.alias : models.Note.encodeNoteId(note.id)))
  }).catch(function (err) {
    logger.error(err)
    return errors.errorInternalError(res)
  })
