  ...
  noteUtil.findNote(req, res, function (note) {
    ...
    switch (action) {
      case 'download':
        ...
        break
      case 'edit':
        res.redirect(config.get('serverURL') + '/' + (note.alias ? note.alias : models.Note.encodeNoteId(note.id)) + '?both')
        break
      default:
        res.redirect(config.get('serverURL') + '/s/' + note.shortid)
        break
    }
  })
}
...
exports.showPublishNote = function (req, res, next) {
  ...
  noteUtil.findNote(req, res, function (note) {
    ...
    if ((note.alias && shortid !== note.alias) || (!note.alias && shortid !== note.shortid)) {
      return res.redirect(config.get('serverURL') + '/s/' + (note.alias || note.shortid))
    }
...
exports.showNote = function (req, res, next) {
  noteUtil.findNote(req, res, function (note) {
    ...
    if ((note.alias && noteId !== note.alias) || (!note.alias && noteId !== id)) {
      return res.redirect(config.get('serverURL') + '/' + (note.alias || id))
    }
...
exports.createFromPOST = function (req, res, next) {
  let body = ''
  if (req.body && req.body.length > config.get('documentMaxLength')) {
  ...
  noteUtil.findNote(req, res, function (note) {
    ...
    switch (action) {
      ...
      case 'pretty': // pretty deprecated
        res.redirect(config.get('serverURL') + '/s/' + (note.alias || note.shortid))
        break
      case 'slide':
        res.redirect(config.get('serverURL') + '/p/' + (note.alias || note.shortid))
        break
      case 'download':
        exports.downloadMarkdown(req, res, note)
        break
      ...
      case 'revision':
        ...
        break
      default:
        return res.redirect(config.get('serverURL') + '/' + noteId)
    }
  })
}
