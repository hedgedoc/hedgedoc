...
let mattermostStrategy = new OAuthStrategy({
  authorizationURL: config.get('mattermost').baseURL + '/oauth/authorize',
  tokenURL: config.get('mattermost').baseURL + '/oauth/access_token',
  clientID: config.get('mattermost').clientID,
  clientSecret: config.get('mattermost').clientSecret,
  callbackURL: config.get('serverURL') + '/auth/mattermost/callback'
}, passportGeneralCallback)

mattermostStrategy.userProfile = (accessToken, done) => {
  mattermost.setUrl(config.get('mattermost').baseURL)
