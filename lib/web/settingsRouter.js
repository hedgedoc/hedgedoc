  ...
  withUser(req, res, (user) => {
    ...
    if (req.xhr) {
      ...
      res.render('settings/account.ejs', {
        title: i18n.__('Profile for %s', user.displayname),
...
SettingsRouter.post('/settings/account', urlencodedParser, function (req, res) {
  withUser(req, res, async (user) => {
    ...
    // just in case of password change
    const oldPasswordVerified = await user.verifyPassword(req.body.old_password)
    const newPasswordsMatch = req.body.new_password_confirmation === req.body.new_password
    if (oldPasswordVerified && newPasswordsMatch) {
      user.password = req.body.new_password
    } else if (!oldPasswordVerified) {
      user.invalid_password_given = 'old password is incorrect'
    } else if (!newPasswordsMatch) {
      user.invalid_password_given = 'given new passwords don\'t match'
    }
    ...
    user.save().then(() => {
      ...
      res.render('settings/account.ejs', {
        title: i18n.__('Account Settings for %s', user.displayname),
...
SettingsRouter.post('/settings/account/delete/:token?', function (req, res) {
  withUser(req, res, (user) => {
