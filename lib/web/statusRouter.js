...
statusRouter.get('/temp', function (req, res) {
  ...
  if (config.get('allowOrigin').indexOf(host) === -1) {
...
statusRouter.get('/config', function (req, res) {
  var data = {
    ...
    DROPBOX_APP_KEY: config.get('dropbox').appKey,
    allowedUploadMimeTypes: config.get('allowedUploadMimeTypes'),
    linkifyHeaderStyle: config.get('linkifyHeaderStyle')
  }
  res.set({
    'Cache-Control': 'private', // only cache by client
    'X-Robots-Tag': 'noindex, nofollow', // prevent crawling
