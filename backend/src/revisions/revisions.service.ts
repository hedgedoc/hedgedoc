...
export class RevisionsService {
  ...
  async getLatestRevision(noteId: number): Promise<Revision> {
    const revision = await this.knex(TableRevision)
      .select()
      .where(FieldNameRevision.noteId, noteId)
      .orderBy(FieldNameRevision.createdAt, 'desc')
  ...
  async getRevisionUserInfo(revisionId: number): Promise<RevisionUserInfo> {
    const authorUsernamesAndGuestUuids = (await this.knex(TableAuthorshipInfo)
      ...
      .where(FieldNameAuthorshipInfo.revisionUuid, revisionId)) as {
  ...
  async createRevision(
    ...
    const latestRevision =
      noteId === undefined ? null : await this.getLatestRevision(noteId);
