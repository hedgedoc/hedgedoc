...
export class ImgurBackend implements MediaBackend {
  ...
  async saveFile(uuid: string, buffer: Buffer): Promise<string> {
    ...
    try {
      ...
      throw new MediaBackendError(`Could not save file ${uuid} on imgur`);
    }
  }

  async deleteFile(uuid: string, jsonBackendData: string): Promise<void> {
    const backendData = JSON.parse(jsonBackendData) as BackendData;
    if (backendData.deleteHash === null) {
      throw new MediaBackendError(
        `We don't have any delete tokens for file ${uuid} and therefore can't delete this image on imgur`,
      );
    }
    try {
      ...
      this.logger.log(`Deleted file ${uuid} on imgur`, 'deleteFile');
      return;
    } catch (e) {
      this.logger.error(
        `error: ${(e as Error).message}`,
      ...
      throw new MediaBackendError(`Could not delete file '${uuid}' on imgur`);
    }
  }
  ...
  getFileUrl(uuid: string, backendData: string | null): Promise<string> {
    if (backendData === null) {
      throw new MediaBackendError(
        `We don't have any data for file ${uuid} and therefore can't get the url of this image on imgur`,
      );
    }
    const data = JSON.parse(backendData) as BackendData;
