...
const samls = samlNames.map(samlName => {
  return {
    ...
    disableRequestedAuthnContext: process.env[`HD_AUTH_SAML_${samlName}_DISABLEREQUESTEDAUTHNCONTEXT`],
...
const oauth2s = oauth2Names.map(oauth2Name => {
  return {
    ...
    userProfileURL: process.env[`HD_AUTH_OAUTH2_${oauth2Name}_USER_PROFILE_URL`],
    userProfileIdAttr:
    ...
    userProfileDisplayNameAttr:
      process.env[`HD_AUTH_OAUTH2_${oauth2Name}_USER_PROFILE_DISPLAY_NAME_ATTR`],
