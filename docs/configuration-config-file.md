...
| `defaultEditableBy` | `owner`, `signedIn` or `anyone` | Set notes default permission (only applied on signed-in users). |
| `defaultViewableBy` | `owner`, `signedIn` or `anyone` | Set notes default permission (only applied on signed-in users). |
