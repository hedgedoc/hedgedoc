...
| Keycloak  | OIDC        | `https://keycloak.example.com/realms/{realm}`, replace accordingly (Note: For Keycloak versions prior to 17, include `/auth/` in the URL path: `https://keycloak.example.com/auth/realms/{realm}`)   | [Keycloak Docs](https://www.keycloak.org/docs/latest/server_admin/#sso-protocols)                    |
