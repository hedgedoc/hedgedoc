...
| `HD_LOG_SHOW_TIMESTAMP`    | true                   |                          | Specifies if a timestamp should be added to the log statements. Disabling is useful for extern log management (systemd etc.)     |
| `HD_MEDIA_MAX_UPLOAD_SIZE` | 20971520               |                          | The maximum file size that can be uploaded to the server in bytes (defaults to 20 MB).                                           |
