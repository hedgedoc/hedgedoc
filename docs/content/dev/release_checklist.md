...
- [ ] Create release tar ball
  ...
  - `yarn build`
  - `cd ..`
  - `tar cvzf hedgedoc-x.x.x.tar.gz --sort=name --exclude hedgedoc/node_modules --exclude hedgedoc/.git --exclude hedgedoc/.github --exclude hedgedoc/.yarn/cache hedgedoc`
...
- [ ] No problems when starting with
  - [ ] SQLite
  - [ ] Postgres
    - `docker run -d --name=hd1-pg -p 5432:5432 -e POSTGRES_USER=hd1db -e POSTGRES_PASSWORD=hd1db -e POSTGRES_DB=hd1db postgres:latest`
    - config:
```
"db": {
  "username": "hd1db",
...
```
  ...
  - [ ] MariaDB
    - `docker run --name=hd1-mysql -p 3306:3306 -e MARIADB_USER=hd1db --env MARIADB_PASSWORD=hd1db --env MARIADB_DATABASE=hd1db -e MARIADB_RANDOM_ROOT_PASSWORD=true --rm -d mariadb:latest`
    - config:
```
"db": {
  "username": "hd1db",
