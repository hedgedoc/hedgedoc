...
1. Start the frontend with `pnpm run start:dev:test` (or use a test build using
   `pnpm run build:test` which you can start using `pnpm run start`).
   The usage of `:test` is mandatory!
2. Run `pnpm run test:e2e:open` to open the cypress test loader
3. Choose your browser and start a test suite

To run all tests in a headless browser use `pnpm run test:e2e`

### Bundle analysis

You can inspect the generated production-bundle files to look for optimization issues.

1. run `pnpm run analyze`. This will overwrite any existing builds!
