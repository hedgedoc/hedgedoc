 ...
 */
import { updateNoteAliases } from '../../../../../redux/note-details/methods'
 ...
 */
export const useOnAliasesUpdated = (websocketConnection: MessageTransporter): void => {
  ...
  useEffect(() => {
    const listener = websocketConnection.on(
      ...
      ({ payload }) => {
        updateNoteAliases(payload.primaryAlias).catch(
          showErrorNotificationBuilder('common.errorWhileLoading', { name: 'note alias refresh' })
        )
      },
      {
        objectify: true
