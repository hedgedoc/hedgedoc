...
export const MediaBrowserSidebarMenu: React.FC<SpecificSidebarMenuProps> = ({
  ...
  const mediaEntries = useMemo(() => {
    ...
    return value.map((entry) => <MediaEntry entry={entry} key={entry.id} />)
  }, [value, loading, error])

  // TODO Translation and better styling when no uploads available
    ...
    <Fragment>
      ...
      <SidebarMenu expand={expand}>
        <AsyncLoadingBoundary loading={loading} componentName={'MediaBrowserSidebarMenu'} error={error}>
          {mediaEntries}
          {mediaEntries.length === 0 && <span>No media uploads</span>}
