...
export const pinNote = async (primaryAlias: string): Promise<void> => {
  try {
    await setPinnedState(primaryAlias, true)
    const action = pinnedNotesActionsCreator.addPinnedNote(primaryAlias)
    store.dispatch(action)
  } catch (error) {
 ...
 * @param primaryAlias The primary alias of the note to toggle
 * @param isPinned Whether the note should be pinned or unpinned
 */
export const toggleNotePinStatus = async (primaryAlias: string, isPinned: boolean): Promise<void> => {
  if (isPinned) {
